"""Polymarket API Client - CLOB V2 (Post-Cutover 28 April 2026)"""
import requests
import json
from typing import Dict, Optional, Tuple

# CLOB V2 imports: py_clob_client_v2 (underscore!)
...
class PolymarketClient:
    ...
    def init_clob(self):
        """Inisialisasi CLOB V2 client"""
        if self._clob is not None:
            return

        print("[Polymarket] Initializing CLOB V2 client...")
        try:
            # V2: constructor pakai named args, chainId -> chain
            ...
            # Auto generate API credentials dari private key
            print("[Polymarket] Deriving API credentials...")
            self._api_creds = self._clob.create_or_derive_api_key()
    ...
    def place_market_order(self, token_id: str, amount: float, side: str) -> Dict:
        ...
        V2 Changes:
        - No more nonce, feeRateBps, taker fields
        - Uses timestamp (ms) for uniqueness
        - Fees determined at match time by protocol
        """
        if self._clob is None:
            self.init_clob()
        ...
        try:
            ...
            # V2: create_and_post_market_order (bukan create_market_order + post_order)
